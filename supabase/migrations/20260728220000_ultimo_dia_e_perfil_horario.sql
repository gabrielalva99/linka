-- LINKA — os dois últimos itens do catálogo do BI (§ Análises de produto).
--
--   último dia contra a média do período, por modelo
--   perfil horário por linha, contra a média da operação
--
-- O segundo é o "DNA de retenção" do BI, que lá é um radar. Sai aqui como
-- barras por hora com a média da operação marcada por cima: a pergunta é "esta
-- linha atrai em horário diferente do resto?", e barra lado a lado responde
-- isso melhor que radar — radar é bonito e péssimo de comparar.
--
-- ── O dia de hoje fica de fora da comparação ─────────────────────────────────
-- Primeira versão pegava o último dia com dado, que às 9h da manhã é o dia de
-- hoje com uma hora de expediente. Contra dias inteiros, todo modelo apareceria
-- "-80% vs. a média" todas as manhãs, e melhoraria sozinho até a noite. Isso não
-- é informação, é o relógio. Agora compara o último dia COMPLETO, e devolve a
-- data dele para a tela poder dizer qual foi.
