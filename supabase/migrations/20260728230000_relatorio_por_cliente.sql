-- LINKA — o relatorio passa a saber de QUAL cliente esta falando.
--
-- fleet_report nao tinha recorte de cliente. Para o usuario de uma marca isso
-- nunca apareceu, porque o RLS ja entrega so o que e dela. Para quem opera a
-- plataforma, que enxerga todos por definicao, o relatorio somaria a Motorola
-- com a marca seguinte num numero so - e ninguem desconfia de um total que
-- apenas parece maior.
--
-- A versao de quatro argumentos SAI. Com as duas convivendo, a chamada de quatro
-- parametros fica ambigua para o Postgres e a tela quebra com um "nao consegui
-- montar o relatorio" que ninguem entenderia. Ja aconteceu aqui quando os
-- filtros entraram.
drop function if exists public.fleet_report(int, text, text, text);
...
begin
  ...
  select jsonb_build_object(
    ...
    'aparelhos_fora', (select count(*) from public.devices
      where exclude_from_reports and (p_tenant is null or tenant_id = p_tenant)),
