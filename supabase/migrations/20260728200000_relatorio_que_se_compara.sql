-- LINKA — o relatório deixa de ser uma foto e passa a se comparar.
--
-- Muda o dono da tela: ela não é instrumento interno da ProSolution, é PRODUTO.
-- O BI pode não vir junto — um cliente pode contratar o LINKA sem a camada de
-- análise. Então a régua deixa de ser "dá para operar com isso" e passa a ser o
-- catálogo do BI atual (docs/CONTRATO-DE-DADOS.md §2).
--
-- Entram as leituras que existem lá e aqui não existiam:
--   variação contra o período anterior (número sozinho não se julga)
--   comparativo por dia da semana, com média por dia
--   concentração dia × hora (onde está o pico e o pico secundário)
--   rua contra shopping por hora (canais com curvas diferentes)
--
-- ── Correção de janela que vem junto ─────────────────────────────────────────
-- O início do período era calculado em UTC e comparado com hora LOCAL da loja.
-- Resultado: "últimos 7 dias" começava às 21h do dia anterior e o relatório
-- carregava três horas de movimento de um dia que não era para estar ali.
-- Passava despercebido numa foto; com comparação contra o período anterior
-- viraria diferença inventada. Agora as duas pontas são hora local.
